...
class StudentResponse(StudentBase):
    ...
    created_at: datetime
    updated_at: datetime
    # Password is explicitly excluded from response

class Student(StudentCreate): # Keeping this for backward compat if needed, or strictly better:
    id: str
    created_at: datetime
    updated_at: datetime
...
def register_student(student: StudentCreate, db=Depends(get_db)):
    ...
    cur.execute(
        """INSERT INTO students (id, first_name, last_name, email, phone, password, role, created_at, updated_at) 
           VALUES (%s,%s,%s,%s,%s,%s,%s,%s,%s)""",
        (student_id, student.first_name, student.last_name,
         student.email.lower(), student.phone, student.password, role, now, now)
    )
    db.commit()

    return {**student.dict(), "id": student_id, "role": role, "created_at": now, "updated_at": now}
...
def login_student(login: StudentLogin, db=Depends(get_db)):
    ...
    # Check if 'password' key exists in the row (handle migration cases)
    ...
    if stored_password and stored_password != login.password:
        ...
        # Fallback for old users without password? Or deny?
        # For now, if no password set, we might allow or deny. 
        # Better to deny if we want to enforce auth.
        raise HTTPException(status_code=400, detail="Password setup required")
